...
export default function Contactbut({ variant = "button", colorClass = "", label }) {
    ...
    <>
        ...
        {visible && ReactDOM.createPortal(
        <div
            className={`fixed top-0 right-0 h-full bg-black text-white z-[9999] overflow-y-auto transition-all duration-500 ease-in-out
            ${open ? "translate-x-0" : "translate-x-full"} md:w-[700px] w-full`}
        >
        <button
            onClick={handleClose}
